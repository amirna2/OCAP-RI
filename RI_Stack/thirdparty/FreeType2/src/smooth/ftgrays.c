...
extern int gray_raster_render( PRaster raster, FT_Raster_Params* params ); 
...
#define FT_PACK( a )           ( ( (long)(a)->y << 16 ) + (a)->x )
#define FT_LESS_THAN( a, b )   ( FT_PACK( a ) < FT_PACK( b ) )

#else /* 1 */

#define FT_LESS_THAN( a, b )   ( (a)->y < (b)->y || \
                               ( (a)->y == (b)->y && (a)->x < (b)->x ) )

#endif /* 1 */

#define FT_SWAP_CELLS( a, b, temp )    do             \
                                       {              \
                                         temp = *(a); \
                                         *(a) = *(b); \
                                         *(b) = temp; } while ( 0 )
                   ...
                   int    count )
  {
    ...
    for (;;)
    {
      ...
      if ( len > QSORT_THRESHOLD )
      {
        ...
        if ( FT_LESS_THAN( j, i ) )
          FT_SWAP_CELLS( i, j, temp );

        if ( FT_LESS_THAN( base, i ) )
          FT_SWAP_CELLS( base, i, temp );

        if ( FT_LESS_THAN( j, base ) )
          FT_SWAP_CELLS( base, j, temp );

        for (;;)
        {
          do i++; while ( FT_LESS_THAN( i, base ) );
          do j--; while ( FT_LESS_THAN( base, j ) );
      ...
      else
      {
        ...
        for ( ; i < limit; j = i, i++ )
        {
          for ( ; FT_LESS_THAN( j + 1, j ); j-- )
          {
            FT_SWAP_CELLS( j + 1, j, temp );
            if ( j == base )
              break;
          }
        }
...
#define M_Y    192
                     ...
                     long         pool_size )
  {
    ...
    if ( raster && pool_base && pool_size >= 4096 )
      gray_init_cells( rast, (void *)pool_base, pool_size );
