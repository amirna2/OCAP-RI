    ...
    /* now load the physical font descriptor */
     ...
     /* now, set-up all root face fields */
     {
       ...
       */
       ...
       if (phy_font->auxdataexists)	// Use aux metrics if exists
       {
	   root->ascender = (FT_Short)( FT_MulDiv(phy_font->ascent, phy_font->outline_resolution,
					    phy_font->metrics_resolution));
	   root->descender = -(FT_Short)( FT_MulDiv(phy_font->descent, phy_font->outline_resolution,
					    phy_font->metrics_resolution));
       }
       else				// Otherwise use yMax/yMin
       {
	    root->ascender     = (FT_Short) phy_font->bbox.yMax;
	    root->descender    = (FT_Short) phy_font->bbox.yMin;
       }

       // Changed from before...passes on height and leading. Height calculated again later.
       root->height		= (FT_Short) (root->ascender - root->descender);
       if (phy_font->auxdataexists) // set leading if aux data available
       {
	   root->leading = (FT_Short)( FT_MulDiv(phy_font->leading, phy_font->outline_resolution,
					phy_font->metrics_resolution));
       }
       else
       {
