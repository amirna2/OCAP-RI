...
#define FT_STRUCTURE  PFR_HeaderRec
  ...
  */
                     ...
                     FT_String*  *astring )
  {
	...
	/* 
	 * Original code expects field rounded up to 16-bits,
	 * with NUL padding if necessary.
	 * However, all off the PFR fonts from TrueDoc.com
	 * have a NUL terminator.  While most fields are rounded
	 * to 16 bits, they all aren't (e.g., Tiresias).
	 * So instead, we'll just back up over all '\0' bytes.
	 */
                     ...
                     FT_UInt32    size )
  {
    ...
    phy_font->auxdataexists = 0;    // default is aux metrics not present
    
    /* in certain fonts, the auxiliary bytes contain interesting  */
    /* information. These are not in the specification but can be */
    /* guessed by looking at the content of a few PFR0 fonts      */
    /* Since comments above, aux info is available in MHP spec.   */
    PFR_CHECK( 3 );
	...
	// Case 2 is defined in the MHP docs. The Davic docs define the other
	// cases as just auxdata.
        ...
        switch ( type )
        {
	      ...
	      phy_font->auxdataexists = 1;  // Others use aux metrics.
	      
              q += 10;
              phy_font->ascent  = PFR_NEXT_SHORT(q);
              phy_font->descent = PFR_NEXT_SHORT(q);
              q += 4;	// 32-bit gap reserved (change from older version)
	      phy_font->leading = PFR_NEXT_SHORT(q);
	      q += length - 24;	// was 16...changed as MHP docs say otherwise.

	      // Here we do a quick check if aux data looks bad. We want to try
	      // and get the best data possible.
	      if (phy_font->ascent <= 0 || phy_font->descent <= 0
		      || phy_font->leading < 0)
	      {
		  phy_font->auxdataexists = 0;	// don't use it if looks wrong
	      }
            }
            break;
          ...
          default:		    // Changed following MHP spec
	    {
		q += length - 4;    // reserved
		break;
	    }
