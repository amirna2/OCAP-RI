...
#define REGION_CODE(x,y,cx1,cx2,cy1,cy2) (unsigned char)( ( (y) > (cy2) ? 8 : 0) | \
                                           ( (y) < (cy1) ? 4 : 0) | \
                                           ( (x) > (cx2) ? 2 : 0) | \
                                           ( (x) < (cx1) ? 1 : 0) )
...
// Returns a bitmap as to what edges of the rect region are intersected by the
// clip region. Note that rect is changed accordingly.
...
// Function returns true if the clip region intersects the rect region. Note
// that in this case, the rect is unchanged. Note that if at a later stage we
// need a function that returns the edges like dfb_clip_rectangle without
// changing the rect, then it can be quickly derived from dfb_clip_rectangle.
unsigned int dfb_clip_check( const DFBRegion *clip, const DFBRectangle *rect )
{
     return (unsigned int)
         (clip->x1 != clip->x2 
          && clip->y1 != clip->y2 
          && (clip->x1 < rect->x + rect->w) 
          && (clip->x2 >= rect->x) 
          && (clip->y1 < rect->y + rect->h) 
          && (clip->y2 >= rect->y)); 
}
...
{
     if (clip->x1 > *dx ) {
          srect->w = DFB_MIN( (clip->x2 - clip->x1) + 1,
                    (*dx + srect->w) - clip->x1);
     ...
     if (clip->y1 > *dy ) {
          srect->h = DFB_MIN( (clip->y2 - clip->y1) + 1,
                          (*dy + srect->h) - clip->y1);
          srect->y+= clip->y1 - *dy;
          *dy = clip->y1;
     }
