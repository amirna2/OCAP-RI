...
LIBEXPORT DFBSurfacePixelFormat dfb_pixelformat_for_depth( int depth );

__u32 dfb_color_to_pixel( DFBSurfacePixelFormat format,
                          __u8 r, __u8 g, __u8 b );

#define span_a1_to_argb(src, dst, width)                                   \
{															\
     int i;													\
     for (i = 0; i < width; i++)									\
...
#define span_a1_to_a8(src, dst, width)								\
{															\
     int i;													\
     for (i = 0; i < width; i++)									\
          *dst++ = (__u8)((src[i>>3] & (1<<(7-(i%8)))) ? 0xFF : 0x0);			\
}

#define span_a8_to_argb(src, dst, width)							\
{															\
     while (width--) *dst++ = PIXEL_ARGB( *src++, 0xFF, 0xFF, 0xFF );		\
}

#define span_rgb15_to_rgb16(src, dst, width)							\
{															\
     while (width--) *dst++ = RGB15_TO_RGB16( *src ), src++;			\
}

#define span_rgb15_to_rgb32(src, dst, width)							\
{															\
     while (width--) *dst++ = RGB15_TO_RGB32( *src ), src++;			\
}

#define span_rgb15_to_argb(src, dst, width)							\
{															\
     while (width--) *dst++ = RGB15_TO_ARGB( *src ), src++;				\
}

#define span_rgb16_to_rgb15(src, dst, width)							\
{															\
     while (width--) *dst++ = RGB16_TO_RGB15( *src ), src++;			\
}

#define span_rgb16_to_rgb32(src, dst, width)							\
{															\
     while (width--) *dst++ = RGB16_TO_RGB32( *src ), src++;			\
}

#define span_rgb16_to_argb(src, dst, width)							\
{															\
     while (width--) *dst++ = RGB16_TO_ARGB( *src ), src++;				\
}

#define span_rgb32_to_rgb332(src, dst, width)						\
{															\
     while (width--) *dst++ = RGB32_TO_RGB332( *src ), src++;			\
}

#define span_rgb32_to_rgb15(src, dst, width)							\
{															\
     while (width--) *dst++ = RGB32_TO_RGB15( *src ), src++;			\
}

#define span_rgb32_to_argb1555(src, dst, width)						\
{															\
     while (width--) *dst++ = RGB32_TO_ARGB1555( *src ), src++;			\
}

#define span_rgb32_to_rgb16(src, dst, width)							\
{															\
     while (width--) *dst++ = RGB32_TO_RGB16( *src ), src++;			\
}

#define span_rgb32_to_argb(src, dst, width)							\
{															\
     while (width--) *dst++ = RGB32_TO_ARGB( *src ), src++;				\
}

#define span_argb_to_a8(src, dst, width)							\
{															\
     while (width--) *dst++ = *src >> 24, src++;						\
}

#define span_argb_to_rgb15(src, dst, width)							\
{															\
     while (width--) *dst++ = RGB32_TO_RGB15( *src ), src++;			\
}

#define span_argb_to_rgb16(src, dst, width)							\
{															\
     while (width--) *dst++ = RGB32_TO_RGB16( *src ), src++;			\
}

#define span_argb_to_rgb32(src, dst, width)							\
{															\
