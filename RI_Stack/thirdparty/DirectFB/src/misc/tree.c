...
DFBResult dfb_tree_insert (Tree *tree,
                      void *key,
                      void *value)
{
     int inserted = 0;
     ...
     if (fast_key >= 0 && fast_key < 96)
          ...
          if ((tree->root = tree_node_insert(tree, tree->root, key, value, &inserted)) == NULL)
                             ...
                             void *value)
{
     ...
     DFB_UNUSED_PARAM(tree);

     if ((node = DFBMALLOC(sizeof (Node))) == NULL)
         return NULL;
                                ...
                                int  *inserted)
{
     ...
     if (cmp < 0) {
          if (node->left) {
               ...
               if ((node->left = tree_node_insert (tree, node->left, key, value, inserted)) == NULL)
                   return NULL;
               ...
               *inserted = 1;
               if ((node->left = tree_node_new (tree, key, value)) == NULL)
                   return NULL;
               node->balance -= 1;
          }
     }
          ...
          if (node->right) {
               ...
               if ((node->right = tree_node_insert (tree, node->right, key, value, inserted)) == NULL)
                   return NULL;
               ...
               *inserted = 1;
               if ((node->right = tree_node_new (tree, key, value)) == NULL)
                   return NULL;
               node->balance += 1;
          }
     }
