                                           ...
                                           CoreSurface **surface )
{
     ...
     if ((s->front_buffer = (SurfaceBuffer *) shcalloc( 1, sizeof(SurfaceBuffer) )) == NULL)
	 ...
	 s->front_buffer->video.native_surface_context_valid = false;
	 s->front_buffer->video.native_surface_context = 0;

     if (caps & DSCAPS_FLIPPING) {
          if ((s->back_buffer = (SurfaceBuffer *) shcalloc( 1, sizeof(SurfaceBuffer) )) == NULL)
              return DFB_NOSYSTEMMEMORY;
		  ...
		  s->back_buffer->video.native_surface_context_valid = false;
		  s->back_buffer->video.native_surface_context = 0;
     }
     else
          s->back_buffer = s->front_buffer;
...
DFBResult dfb_surface_create_preallocated_video( int width, int height,
                                                 DFBSurfacePixelFormat format,
                                                 CoreSurfacePolicy policy, DFBSurfaceCapabilities caps,
                                                 CorePalette *palette,
                                                 void *front_buffer, void *back_buffer,
										         ...
										         unsigned long native_surface_context, CoreSurface **surface )
{
	 ...
	 s->front_buffer->video.health = CSH_STORED;
	 s->front_buffer->video.pitch  = front_pitch;
	 s->front_buffer->video.offset = (int)front_buffer;

	 s->front_buffer->video.native_surface_context_valid = true;
	 s->front_buffer->video.native_surface_context = native_surface_context;
     ...
     if (caps & DSCAPS_FLIPPING) {
          ...
          s->back_buffer->video.pitch  = back_pitch;
          s->back_buffer->video.offset = (int)back_buffer;

		  s->back_buffer->video.native_surface_context_valid = false;	/* FIXME: should return an error if (caps & DSCAPS_FLIPPING) */
		  s->back_buffer->video.native_surface_context = 0;
     }
     else
          s->back_buffer = s->front_buffer;
                         ...
                         CorePalette *palette )
{
     ...
     if (palette) {
          ...
          (void) dfb_palette_attach_global( palette, DFB_SURFACE_PALETTE_LISTENER,
                                 ...
                                 void **data, int *pitch, int front )
{
     ...
     dfb_surfacemanager_lock( surface->manager );
     ret = dfb_surface_software_lock( surface, flags, data, (unsigned int *)pitch, front );
     dfb_surfacemanager_unlock( surface->manager );
                                              ...
                                              SurfaceBuffer     **ret_buffer )
{
     ...
     if ((buffer = shcalloc( 1, sizeof(SurfaceBuffer) )) == NULL)
         return DFB_NOSYSTEMMEMORY;

     buffer->policy  = policy;
     buffer->surface = surface;

	/* Initially there is no native surface context associated with the surface buffer.     */
	/* For the CSP_VIDEOONLY case below, dfb_surfacemanager_allocate() may over-ride this   */
	/* if it is able to create a compatible native buffer.                                  */

	 buffer->video.native_surface_context_valid = false;
	 buffer->video.native_surface_context = 0;
