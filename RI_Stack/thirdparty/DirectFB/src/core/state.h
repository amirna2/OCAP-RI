...
typedef enum {
	 ...
	 SMF_ALPHA_CONST	 = 0x00000400,
	 SMF_PORTER_DUFF	 = 0x00000800,
...
struct _CardState {
	 ...
	 __u8					 alpha_const;  /* alpha const to adjust src alpha */
     ...
     DFBAccelerationMask     set;          /* commands for which a state has
                                              been set */
	 // Booleans indicating if the destination rectangle has a negative width
	 // or height respectively. Note that source rectangles can be translated to
	 // the destination when it comes to sign. It is up to the driver for the
	 // specific hardware to handle negative reflective widths or heights
	 // properly. The software driver does so.
	 bool					flagwneg;
	 bool					flaghneg;
...
#define dfb_state_lock(state) pthread_mutex_lock( &(state)->lock )
#define dfb_state_unlock(state) pthread_mutex_unlock( &(state)->lock )
