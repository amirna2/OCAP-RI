                           ...
                           unsigned int pixelpitch_align )
{
	...
	DFB_UNUSED_PARAM(byteoffset_align);
	DFB_UNUSED_PARAM(pixelpitch_align);
                                       ...
                                       SurfaceBuffer  *buffer )
{
	...
	if (dfb_gfxcard_createsurface(&surface_context, &surface_address, &surface_pitch, surface->format, surface->width, surface->height))
	{
		buffer->video.native_surface_context_valid = true;
		buffer->video.native_surface_context = surface_context;

		buffer->video.health = CSH_RESTORE;
		buffer->video.offset = (int)surface_address;
		buffer->video.pitch = surface_pitch;
                                         ...
                                         SurfaceBuffer  *buffer )
{
	...
	if (buffer->video.native_surface_context_valid)
	{
		dfb_gfxcard_deletesurface(buffer->video.native_surface_context);
	}
                                           ...
                                           SurfaceBuffer  *buffer )
{
	...
	switch (buffer->video.health) {
		...
		case CSH_RESTORE: {
			...
			while (h--) {
				dfb_memcpy(dst, src, DFB_BYTES_PER_LINE(surface->format, surface->width));
			...
			if (DFB_PLANAR_PIXELFORMAT(surface->format)) {
				h = surface->height;
				while (h--) {
					dfb_memcpy(dst, src, DFB_BYTES_PER_LINE(surface->format, surface->width / 2));
...
static DFBResult occupy_chunk( SurfaceManager *manager, Chunk *chunk, SurfaceBuffer *buffer, int length );
                                                 ...
                                                 unsigned int    offset )
{
     ...
     if (manager->chunks->buffer == NULL) {
          /* first chunk is free */
          if ((int)offset <= manager->chunks->offset + manager->chunks->length) {
                                       ...
                                       SurfaceBuffer  *buffer )
{
     ...
     pitch = DFB_MAX( surface->width, surface->min_width );
     ...
     if (length > manager->available - (int)manager->heap_offset)
          return DFB_NOVIDEOMEMORY;
     ...
     if (best_free) {
          ...
          */
          ...
          if ((ret = occupy_chunk( manager, best_free, buffer, length )) != DFB_OK)
              return ret;
     } else
     if (best_occupied) {
          ...
          if ((ret = occupy_chunk( manager, best_occupied, buffer, length )) != DFB_OK)
              return ret;
     }
     else {
...
static Chunk* split_chunk( Chunk *c, int length )
{
     ...
     if ((newchunk = (Chunk*) shcalloc( 1, sizeof(Chunk) )) == NULL)
         return newchunk;
...
occupy_chunk( SurfaceManager *manager, Chunk *chunk, SurfaceBuffer *buffer, int length )
{
     ...
     if ((chunk = split_chunk( chunk, length )) == NULL)
