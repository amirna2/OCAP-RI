                ...
                CoreGlyphData *info )
{
     ...
     info->width   = face->glyph->bitmap.width;
     info->height  = face->glyph->bitmap.rows;
     info->advance = data->fixed_advance ? data->fixed_advance : (face->glyph->advance.x >> 6);
...
Probe( IDirectFBFont_ProbeContext *ctx )
{
     ...
     if (ctx->filename)
         ...
         if (ctx->fontbuffer)
             err = FT_New_Memory_Face( library, (const FT_Byte*)(ctx->fontbuffer), ctx->fontsize, -1, &face );
         else
           ...
           DFBFontDescription *desc )
{
     ...
     if (filename != NULL)
         ...
         err = FT_New_Memory_Face( library, (const FT_Byte *)fontbuffer, fontsize,
                            (desc->flags & DFDESC_INDEX) ? desc->index : 0,
                            &face );
     if (err) {
          switch (err) {
