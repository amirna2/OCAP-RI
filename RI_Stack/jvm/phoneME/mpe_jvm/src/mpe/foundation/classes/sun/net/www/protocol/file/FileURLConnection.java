/*
 * @(#)FileURLConnection.java	1.57 06/10/10
 *
...
import org.cablelabs.impl.io.FileIntrospector;
...
public class FileURLConnection extends URLConnection
{
    ...
    /*
     * Note: the semantics of FileURLConnection object is that the results of
     * the various URLConnection calls, such as getContentType, getInputStream
     * or getContentLength reflect whatever was true when connect was called.
     */
    ...
    private void initializeHeaders()
    {
        ...
        if (!initializedHeaders || !exists)
        {
            ...
            if (!isDirectory)
            {
                ...
                // First check OCAP filesystem for contentType
                FileSys fs = FileSysManager.getFileSys(filename);
                contentType = fs.contentType(filename);

                // Next, use JVM file name map just like traditional operation
                if (contentType == null)
                {
                ...
                // Finally, try to look into the file contents to determine the
                // type
                if (contentType == null)
                {
                    try
                    {
                        contentType = (new FileIntrospector(new FileInputStream(filename))).getContentType();
                    }
                ...
                /*
                 * Format the last-modified field into the preferred Internet
                 * standard - ie: fixed-length subset of that defined by RFC
                 * 1123
                 */
                if (lastModified != 0)
                {
    ...
    public synchronized InputStream getInputStream() throws IOException
    {
        ...
        if (is == null)
        {
            iconHeight = ((Integer) java.security.AccessController.doPrivileged(new GetIntegerAction(
                    "hotjava.file.iconheight", 32))).intValue();
            iconWidth = ((Integer) java.security.AccessController.doPrivileged(new GetIntegerAction(
                    "hotjava.file.iconwidth", 32))).intValue();
            if (isDirectory)
            {
                FileNameMap map = java.net.URLConnection.getFileNameMap();
                ...
                buf.append("<title>");
                buf.append((String) java.security.AccessController.doPrivileged(new GetPropertyAction("file.dir.title",
                        "Directory Listing")));
                buf.append("</title>\n");
                ...
                for (int i = 0; i < files.size(); i++)
                {
                    ...
                    {
                        buf.append(MimeEntry.defaultImagePath + "/directory.gif\" width=" + iconWidth + " height="
                                + iconHeight + ">\n");
                    }
                    else
                    {
