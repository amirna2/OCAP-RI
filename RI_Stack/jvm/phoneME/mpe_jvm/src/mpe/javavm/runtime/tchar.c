...
WCHAR *createWCHAR(const char *string)
{
    ...
    size = MultiByteToWideChar(CP_UTF8, 0, string, -1, NULL, 0);
    if (size == 0)
    {
        return NULL;
    }
    ...
    size = MultiByteToWideChar(CP_UTF8, 0, string, -1, ws_temp, size);
    if (size == 0)
    {
        free(ws_temp);
        return NULL;
...
char* convertWtoU8(char* astring, const WCHAR* u16string)
{
    int size;
    /* calculating the required buffer size */
    size = WideCharToMultiByte(CP_UTF8, 0, u16string, -1, NULL, 0, NULL, NULL);
    if (size == 0)
    {
        return NULL;
    }
    ...
    /* converting from wide string to UTF-8 */
    size = WideCharToMultiByte(CP_UTF8, 0, u16string, -1, astring, size, NULL,
            NULL);
    if (size == 0)
    {
        free(astring);
...
char* convertWtoA(char* astring, const WCHAR* u16string)
{
    int size;
    /* calculating the required buffer size */
    size = WideCharToMultiByte(CP_ACP, MB_PRECOMPOSED, u16string, -1, NULL, 0,
            NULL, NULL);
    if (size == 0)
    {
        return NULL;
    ...
    /* converting from wide string to UTF-8 */
    size = WideCharToMultiByte(CP_ACP, MB_PRECOMPOSED, u16string, -1, astring,
            size, NULL, NULL);
    if (size == 0)
    {
        free(astring);
...
char* createU8fromA(const char* astring)
{
    ...
    char* u8string;
    /* calculating the required buffer size */
    size = MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, astring, -1, NULL, 0);
    ws_temp = malloc(size * sizeof(WCHAR));
    /* converting from local to wide string */
    size = MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, astring, -1, ws_temp,
            size);
    if (size == 0)
    {
        free(ws_temp);
