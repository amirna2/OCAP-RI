 ...
 *******************************************************************************
 *
 * $Source: /usr/cvs/icu4j/icu4j/src/com/ibm/icu/text/UnicodeCompressor.java,v $ 
 ...
 * {
 *     // do the compression
 *     bytesWritten = myCompressor.compress(chars, totalCharsCompressed, len, unicharsRead, byteBuffer, 0, BUFSIZE);
 * 
...
public final class UnicodeCompressor implements SCSU
{
    ...
    private static boolean[] sSingleTagTable = {
            ...
            false, true, true, true, true, true, true, true, true, false, false, true, true, false, true, true, true,
     ...
     * 
     * @param buffer
     *            The character buffer to compress.
     * @param start
     *            The start of the character run to compress.
     * @param limit
     ...
     * @param charBuffer
     *            The character buffer to compress.
     * @param charBufferStart
     *            The start of the character run to compress.
     * @param charBufferLimit
     *            The limit of the character run to compress.
     * @param charsRead
     *            A one-element array. If not null, on return the number of
     *            characters read from charBuffer.
     * @param byteBuffer
     *            A buffer to receive the compressed data. This buffer must be
     *            at minimum four bytes in size.
     * @param byteBufferStart
     *            The starting offset to which to write compressed data.
     * @param byteBufferLimit
     *            The limiting offset for writing compressed data.
     * @return The number of bytes written to byteBuffer.
     * @draft ICU 2.4
     */
    public int compress(char[] charBuffer, int charBufferStart, int charBufferLimit, int[] charsRead,
            byte[] byteBuffer, int byteBufferStart, int byteBufferLimit)
    {
                    ...
                    singleByteModeLoop: while (ucPos < charBufferLimit && bytePos < byteBufferLimit)
                    {
                        ...
                        if (curUC < 0x0080)
                        {
                            ...
                            if (sSingleTagTable[loByte])
                            {
                                ...
                                // since we know the byte is less than 0x80,
                                // SQUOTE0
                        ...
                        else if ((whichWindow = findStaticWindow(curUC)) != INVALIDWINDOW
        ...
        if (charsRead != null) charsRead[0] = (ucPos - charBufferStart);
     ...
     * @param c
     *            The character in question
     * @return The dynamic window containing <TT>c</TT>, or INVALIDWINDOW if not
     *         defined.
     */
     ...
     * @param c
     *            The character in question
     * @return The static window containing <TT>c</TT>, or INVALIDWINDOW if not
     *         defined.
     */
