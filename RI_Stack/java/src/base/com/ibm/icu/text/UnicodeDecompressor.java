 ...
 *******************************************************************************
 *
 * $Source: /usr/cvs/icu4j/icu4j/src/com/ibm/icu/text/UnicodeDecompressor.java,v $ 
 ...
 *     charsWritten = myDecompressor.decompress(bytes, totalBytesDecompressed, len, bytesRead, charBuffer, 0, BUFSIZE);
 * 
...
public final class UnicodeDecompressor implements SCSU
{
     ...
     *            The byte array to decompress.
     * @param start
     *            The start of the byte run to decompress.
     * @param limit
     ...
     *            bytes read from byteBuffer.
     * @param charBuffer
     *            A buffer to receive the decompressed data. This buffer must be
     *            at minimum two characters in size.
     * @param charBufferStart
     *            The starting offset to which to write decompressed data.
     ...
     * @draft ICU 2.4
     */
    public int decompress(byte[] byteBuffer, int byteBufferStart, int byteBufferLimit, int[] bytesRead,
            char[] charBuffer, int charBufferStart, int charBufferLimit)
    {
        ...
        if (fBufferLength > 0)
        {
            ...
            if (fBufferLength != BUFSIZE)
            {
                ...
                if (byteBufferLimit - byteBufferStart < newBytes) newBytes = byteBufferLimit - byteBufferStart;
                ...
                case SINGLEBYTEMODE:
                    ...
                    singleByteModeLoop: while (bytePos < byteBufferLimit && ucPos < charBufferLimit)
                    {
                        ...
                        switch (aByte)
                        {
                            ...
                            case 0xFE:
                            case 0xFF:
                                // For offsets <= 0xFFFF, convert to a single
                                // char
                                ...
                                // For offsets > 0x10000, convert to a surrogate
                                // pair by
                                // normBase = window's offset - 0x10000
                                // high surr. = 0xD800 + (normBase >> 10)
                                // low surr. = 0xDC00 + (normBase & 0x3FF) +
                                // (byte & 0x7F)
                                else
                                {
                            ...
                            // (this is the same as quoting from static window
                            // 0)
                            ...
                            case SQUOTE7:
                                ...
                                // static window n otherwise, use dynamic window
                                // n
                                int dByte = byteBuffer[bytePos++] & 0xFF;
                                charBuffer[ucPos++] = (char) (dByte + (dByte >= 0x00 && dByte < 0x80 ? sOffsets[aByte
                                        - SQUOTE0] : (fOffsets[aByte - SQUOTE0] - COMPRESSIONOFFSET)));
                                break;
