...
public class Roller {
  ...
  private static final Logger log = Logger.getLogger(Roller.class);
  ...
  void roll() {
    try {
      ...
      if(ExternallyRolledFileAppender.OK.equals(rc)) {
        ...
        {
          log.info("Roll over signal acknowledged by remote appender.");
        }
      } else {
          if (log.isWarnEnabled())
          {
            log.warn("Unexpected return code "+rc+" from remote entity.");
          }
      }
    } catch(IOException e) {
      log.error("Could not send roll signal on host "+host+" port "+port+" .",
		e);
    }
  }
}
