...
public class PropertyConfigurator implements Configurator {
  ...
  static final String GROUP_PREFIX         = "log4j.group.";
  ...
  private static final String GROUPS_KEY = "log4j.groups";
  ...
  */
  ...
  void doConfigure(String configFileName, LoggerRepository hierarchy) {
    validateLoggerConfigPermission();
  ...
  */
  ...
  void doConfigure(Properties properties, LoggerRepository hierarchy) {
    ...
    //define groups after explicit loggers have been constructed (so new loggers not defined explicitly will get the group threshold)
    configureGroups(properties, hierarchy);
    ...
    private void configureGroups(Properties props, LoggerRepository hierarchy)
    {
        ...
        while (enumeration.hasMoreElements())
        {
            ...
            {
                //prefix includes trailing dot
                String groupName = key.substring(GROUP_PREFIX.length());
                Level groupThreshold = Level.toLevel(OptionConverter.findAndSubst(key, props));
                LogLog.debug("Setting group: " + groupName + " to: " + groupThreshold);
                hierarchy.setGroup(groupName, groupThreshold);
            }
        }
    }
   ...
   */
  private void configureLoggerFactory(Properties props) {
  ...
  */
  private void parseCatsAndRenderers(Properties props, LoggerRepository hierarchy)
  {
      Enumeration enumeration = props.propertyNames();
      while (enumeration.hasMoreElements())
      {
  ...
  private static void validateLoggerConfigPermission()
  {
      SecurityUtil.checkPermission(new MonitorAppPermission("logger.config"));
  }
}
