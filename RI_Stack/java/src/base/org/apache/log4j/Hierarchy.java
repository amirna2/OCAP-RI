 ...
 */
  ...
  Logger getLogger(String name, String group) {
    return getLogger(name, group, defaultFactory);
  }

    public void setGroup(String groupName, Level groupThreshold)
    {
        groups.put(groupName, groupThreshold);
    }

    public Logger getLogger(String name, LoggerFactory factory)
    {
        return getLogger(name, null, factory);
    }
 ...
 */
  ...
  Logger getLogger(String name, String group, LoggerFactory factory) {
    ...
    synchronized(ht) {
      Object o = ht.get(key);
        //only apply the 'group' level if an explicit logger was not already constructed via the configuration process
      if(o == null) {
	logger = factory.makeNewLoggerInstance(name);
	logger.setHierarchy(this);
    //group may be null
    if (group != null && groups != null && groups.containsKey(group))
    {
        Level level = (Level) groups.get(group);
        logger.setLevel(level);
    }
    //always set group even if it is null or wasn't in the set of groups explicitly configured
    logger.setGroup(group);

	ht.put(key, logger);
    //even though this logger may have a level assigned because of a group definition, if there is a parent logger
    //defined with a level, that level needs to take precedence
	updateParents(logger);
	return logger;
      } else if(o instanceof Logger) {
        //ensure group is set before returning it (it may have been explicitly set via configuration so it would be pre-existing)
        ((Logger)o).setGroup(group);
	return (Logger) o;
  ...
  */
  ...
  void updateParents(Logger cat) {
    ...
    //a parent was found - update the level (cat's level may have been previously set due to a group definition, but a parent exists
    cat.level = ((Category)o).level;
