...
public final class RuleFactory {
  ...
  private static final String LIKE_RULE_CLASS_NAME = "org.apache.log4j.rule.LikeRule";
  ...
  static {
    ...
    RULES.add(PARTIAL_TEXT_MATCH_RULE);
      try
      {
          Class.forName(LIKE_RULE_CLASS_NAME);
          RULES.add(LIKE_RULE);
      } catch (Throwable t)
      {
          LogLog.warn("Unable to load like rule class", t);
      }
  ...
  public Rule getRule(final String symbol, final Stack stack) {
    ...
    if (RULES.contains(LIKE_RULE) && LIKE_RULE.equalsIgnoreCase(symbol)) {
        try
        {
          Class likeRuleClass = Class.forName(LIKE_RULE_CLASS_NAME);
          Method getRuleMethod = likeRuleClass.getMethod("getRule", new Class[]{Stack.class});
          return (Rule) getRuleMethod.invoke(likeRuleClass, new Object[]{stack});
        } catch (ClassNotFoundException e)
        {
            LogLog.warn("Unable to load like rule", e);
