...
/**
 * Entity for NetModule Event. There are two types of NetModule events: one that
 * is generated by the NetManager when a NetModule is added or removed from the
 * home network. Application may register as a listener to NetManager to receive
 * such events. The other NetModuleEvent is generated by the NetModule itself
 * when its internal state changes. Application should register as a listener
 * with a particular NetModule for such events. In both scenarios, the NetModule
 * that was the source of the event is returned.
 */
