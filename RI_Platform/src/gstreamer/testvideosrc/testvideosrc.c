        ...
        unsigned char color)
{
    ...
    for (i = 0; i < h; i++)
    {
        /*lint -e(611)*/oil_splat_u8_ns (d, &color, w);
        d += stride;
    }
}
#endif
...
static int y_colors[] =
{   255, 226, 179, 150, 105, 76, 29, 16, 16, 16, 0, 32};
static int u_colors[] =
...
/*                        wht  yel  cya  grn  mag  red  blu  blk   -I    Q  superblack, dark grey */
static int r_colors[] =
{   255, 255, 0, 0, 255, 255, 0, 0, 0, 0, 0, 32};
static int g_colors[] =
{   255, 255, 255, 255, 0, 0, 0, 0, 0, 128, 0, 32};
static int b_colors[] =
{   255, 0, 255, 0, 255, 0, 255, 0, 128, 255, 0, 32};
#endif
...
struct fourcc_list_struct
        ...
        {
                /* packed */
                { VTS_YUV, "YUY2", "YUY2", 16, paint_setup_YUY2,
                        paint_hline_YUY2 },
                { VTS_YUV, "UYVY", "UYVY", 16, paint_setup_UYVY,
                        paint_hline_YUY2 },
                { VTS_YUV, "Y422", "Y422", 16, paint_setup_UYVY,
                        paint_hline_YUY2 },
                { VTS_YUV, "UYNV", "UYNV", 16, paint_setup_UYVY,
                        paint_hline_YUY2 }, /* FIXME: UYNV? */
                { VTS_YUV, "YVYU", "YVYU", 16, paint_setup_YVYU,
                        paint_hline_YUY2 },
                { VTS_YUV, "AYUV", "AYUV", 32, paint_setup_AYUV,
                        paint_hline_AYUV },
                ...
                { VTS_YUV, "IYU2", "IYU2", 24, paint_setup_IYU2,
                        paint_hline_IYU2 },

                /* planar */
                /* YVU9 */
                { VTS_YUV, "YVU9", "YVU9", 9, paint_setup_YVU9,
                        paint_hline_YUV9 },
                /* YUV9 */
                { VTS_YUV, "YUV9", "YUV9", 9, paint_setup_YUV9,
                        paint_hline_YUV9 },
                /* IF09 */
                /* YV12 */
                { VTS_YUV, "YV12", "YV12", 12, paint_setup_YV12,
                        paint_hline_I420 },
                /* I420 */
                { VTS_YUV, "I420", "I420", 12, paint_setup_I420,
                        paint_hline_I420 },
                /* NV12 */
                { VTS_YUV, "NV12", "NV12", 12, paint_setup_NV12,
                        paint_hline_NV12_NV21 },
                /* NV21 */
                { VTS_YUV, "NV21", "NV21", 12, paint_setup_NV21,
                        paint_hline_NV12_NV21 },
#if 0
                /* IMC1 */
                ...
                /* CLPL */
                /* Y41B */
                { VTS_YUV, "Y41B", "Y41B", 12, paint_setup_Y41B,
                        paint_hline_Y41B },
                /* Y42B */
                { VTS_YUV, "Y42B", "Y42B", 16, paint_setup_Y42B,
                        paint_hline_Y42B },
                /* Y800 grayscale */
                { VTS_YUV, "Y800", "Y800", 8, paint_setup_Y800,
                        paint_hline_Y800 },

                { VTS_RGB, "RGB ", "xRGB8888", 32, paint_setup_xRGB8888,
                        paint_hline_str4, 24, 0x00ff0000, 0x0000ff00,
                        0x000000ff },
                { VTS_RGB, "RGB ", "xBGR8888", 32, paint_setup_xBGR8888,
                        paint_hline_str4, 24, 0x000000ff, 0x0000ff00,
                        0x00ff0000 },
                { VTS_RGB, "RGB ", "RGBx8888", 32, paint_setup_RGBx8888,
                        paint_hline_str4, 24, 0xff000000, 0x00ff0000,
                        0x0000ff00 },
                { VTS_RGB, "RGB ", "BGRx8888", 32, paint_setup_BGRx8888,
                        paint_hline_str4, 24, 0x0000ff00, 0x00ff0000,
                        0xff000000 },
                { VTS_RGB, "RGB ", "ARGB8888", 32, paint_setup_ARGB8888,
                        paint_hline_str4, 32, 0x00ff0000, 0x0000ff00,
                        0x000000ff, 0xff000000 },
                { VTS_RGB, "RGB ", "ABGR8888", 32, paint_setup_ABGR8888,
                        paint_hline_str4, 32, 0x000000ff, 0x0000ff00,
                        0x00ff0000, 0xff000000 },
                { VTS_RGB, "RGB ", "RGBA8888", 32, paint_setup_RGBA8888,
                        paint_hline_str4, 32, 0xff000000, 0x00ff0000,
                        0x0000ff00, 0x000000ff },
                { VTS_RGB, "RGB ", "BGRA8888", 32, paint_setup_BGRA8888,
                        paint_hline_str4, 32, 0x0000ff00, 0x00ff0000,
                        0xff000000, 0x000000ff },
                { VTS_RGB, "RGB ", "RGB888", 24, paint_setup_RGB888,
                        paint_hline_str3, 24, 0x00ff0000, 0x0000ff00,
                        0x000000ff },
                { VTS_RGB, "RGB ", "BGR888", 24, paint_setup_BGR888,
                        paint_hline_str3, 24, 0x000000ff, 0x0000ff00,
                        0x00ff0000 },
                { VTS_RGB, "RGB ", "RGB565", 16, paint_setup_RGB565,
                        paint_hline_RGB565, 16, 0x0000f800, 0x000007e0,
                        0x0000001f },
                { VTS_RGB, "RGB ", "xRGB1555", 16, paint_setup_xRGB1555,
                        paint_hline_xRGB1555, 15, 0x00007c00, 0x000003e0,
                        0x0000001f },

                { VTS_BAYER, "BAY8", "Bayer", 8, paint_setup_bayer,
                        paint_hline_bayer } };
...
paintinfo_find_by_structure(const GstStructure * structure)
{
    ...
    else if (strcmp(media_type, "video/x-raw-rgb") == 0)
    {
        ...
        ret = (ret && gst_structure_get_int(structure, "green_mask",
                &green_mask));
        ret
                = (ret && gst_structure_get_int(structure, "blue_mask",
                        &blue_mask));
        ret = (ret && gst_structure_get_int(structure, "depth", &depth));
        ret = (ret && gst_structure_get_int(structure, "bpp", &bpp));

        if (depth == 32)
        {
            ret = (ret && gst_structure_get_int(structure, "alpha_mask",
                    &alpha_mask));
            ret = (ret && (alpha_mask != 0));
        }
        else
        {
            alpha_mask = 0;
        ...
        for (i = 0; i < n_fourccs; i++)
        {
            if (fourcc_list[i].type == VTS_RGB && fourcc_list[i].red_mask
                    == red_mask && fourcc_list[i].green_mask == green_mask
                    && fourcc_list[i].blue_mask == blue_mask && (alpha_mask
                    == 0 || fourcc_list[i].alpha_mask == alpha_mask)
                    && fourcc_list[i].depth == depth && fourcc_list[i].bitspp
                    == bpp)
            {
                return fourcc_list + i;
            }
...
paint_get_structure(struct fourcc_list_struct * format)
{
    ...
    fourcc = GST_MAKE_FOURCC(format->fourcc[0], format->fourcc[1],
            format->fourcc[2], format->fourcc[3]);
    ...
    case VTS_RGB:
        ...
        structure = gst_structure_new("video/x-raw-rgb", "bpp", G_TYPE_INT,
                format->bitspp, "endianness", G_TYPE_INT, endianness, "depth",
                G_TYPE_INT, format->depth, "red_mask", G_TYPE_INT,
                format->red_mask, "green_mask", G_TYPE_INT, format->green_mask,
        ...
        break;
    case VTS_YUV:
        structure = gst_structure_new("video/x-raw-yuv", "format",
 ...
 * given the fourcc in GstTestVideoSrc */
...
static void gst_test_video_src_unicolor(GstTestVideoSrc * v,
        unsigned char *dest, int w, int h, const struct vts_color_struct *color)
{
    int i;
    paintinfo pi =
    { NULL, };
...
void gst_test_video_src_black(GstTestVideoSrc * v, guchar * dest, int w, int h)
{
    gst_test_video_src_unicolor(v, dest, w, h, vts_colors + COLOR_BLACK);
}

void gst_test_video_src_white(GstTestVideoSrc * v, guchar * dest, int w, int h)
{
    gst_test_video_src_unicolor(v, dest, w, h, vts_colors + COLOR_WHITE);
}

void gst_test_video_src_red(GstTestVideoSrc * v, guchar * dest, int w, int h)
{
    gst_test_video_src_unicolor(v, dest, w, h, vts_colors + COLOR_RED);
}

void gst_test_video_src_green(GstTestVideoSrc * v, guchar * dest, int w, int h)
{
    gst_test_video_src_unicolor(v, dest, w, h, vts_colors + COLOR_GREEN);
}

void gst_test_video_src_blue(GstTestVideoSrc * v, guchar * dest, int w, int h)
{
    gst_test_video_src_unicolor(v, dest, w, h, vts_colors + COLOR_BLUE);
}

void gst_test_video_src_checkers1(GstTestVideoSrc * v, guchar * dest, int w,
        int h)
{
    int x, y;
    paintinfo pi =
...
void gst_test_video_src_checkers2(GstTestVideoSrc * v, guchar * dest, int w,
        int h)
{
    int x, y;
    paintinfo pi =
...
void gst_test_video_src_checkers4(GstTestVideoSrc * v, guchar * dest, int w,
        int h)
{
    int x, y;
    paintinfo pi =
...
void gst_test_video_src_checkers8(GstTestVideoSrc * v, guchar * dest, int w,
        int h)
{
    int x, y;
    paintinfo pi =
        ...
        int w, int h)
{
    ...
    if (!sine_array_inited)
    {
        for (i = 0; i < 256; i++)
        {
            sine_array[i] = (uint8_t) floor(255 * (0.5 + 0.5 * sin(i * 2 * M_PI
                    / 256)) + 0.5);
        }
    ...
    for (i = 0; i < w; i++)
    {
        for (j = 0; j < h; j++)
        {
            ...
            dist = sqrt((2 * i - w) * (2 * i - w) + (2 * j - h) * (2 * j - h))
                    / (2 * w);
            seg = (int) floor(dist * 16);
            if (seg == 0 || seg >= 8)
            {
                color.Y = 255;
            }
            else
            {
                ...
                double a;
#endif
                d = (int) floor(256 * dist * freq[seg] + 0.5);
#ifdef SCALE_AMPLITUDE
                a = ampl[seg];
                if (a < 0)
                a = 0;
...
static void paint_hline_I420(paintinfo * p, int x, int y, int w)
{
    ...
    /*lint -e(611)*/oil_splat_u8_ns(p->vp + offset1 + x1, &p->color->V, x2 - x1);
}
...
static void paint_hline_NV12_NV21(paintinfo * p, int x, int y, int w)
{
    ...
    if (uvlength)
    {
        /*lint -e(611)*/oil_splat_u8(p->up + offsetuv, 2, &p->color->U,
                uvlength);
        /*lint -e(611)*/oil_splat_u8(p->vp + offsetuv, 2, &p->color->V,
                uvlength);
    }
}
...
static void paint_hline_AYUV(paintinfo * p, int x, int y, int w)
{
    ...
    /*lint -e(611)*/oil_splat_u8(p->ap + offset, 4, &p->color->A, w);
}
...
static void paint_hline_YUY2(paintinfo * p, int x, int y, int w)
{
    ...
    /*lint -e(611)*/oil_splat_u8(p->vp + offset + x1 * 4, 4, &p->color->V, x2
            - x1);
}
...
static void paint_hline_IYU2(paintinfo * p, int x, int y, int w)
{
    ...
    /*lint -e(611)*/oil_splat_u8(p->vp + offset + x * 3, 3, &p->color->V, w);
}
...
static void paint_hline_Y800(paintinfo * p, int x, int y, int w)
{
    ...
    /*lint -e(611)*/oil_splat_u8_ns(p->yp + offset + x, &p->color->Y, w);
}
...
paint_hline_IMC1 (paintinfo * p, int x, int y, int w)
{
    ...
    /*lint -e(611)*/oil_splat_u8_ns (p->yp + offset + x, &p->color->Y, w);
    /*lint -e(611)*/oil_splat_u8_ns (p->up + offset1 + x1, &p->color->U, x2 - x1);
    /*lint -e(611)*/oil_splat_u8_ns (p->vp + offset1 + x1, &p->color->V, x2 - x1);
}
#endif
...
static void paint_hline_str4(paintinfo * p, int x, int y, int w)
{
    ...
    if (p->ap != NULL)
    {
        /*lint -e(611)*/oil_splat_u8(p->ap + offset + (x * 4), 4, &p->color->A,
                w);
    }
}
...
static void paint_hline_str3(paintinfo * p, int x, int y, int w)
{
    ...
    /*lint -e(611)*/oil_splat_u8(p->vp + offset + x * 3, 3, &p->color->B, w);
}
...
static void paint_hline_RGB565(paintinfo * p, int x, int y, int w)
{
    ...
    /*lint -e(611)*/oil_splat_u8(p->yp + offset + x * 2 + 1, 2, &a, w);
#else
    /*lint -e(611)*/oil_splat_u8 (p->yp + offset + x * 2 + 0, 2, &a, w);
    /*lint -e(611)*/oil_splat_u8 (p->yp + offset + x * 2 + 1, 2, &b, w);
#endif
}
