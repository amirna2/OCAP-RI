...
#define /*lint -e(652)*/ GST_CAT_DEFAULT audio_test_src_debug

static const GstElementDetails gst_audio_test_src_details =
        GST_ELEMENT_DETAILS("Audio test source", "Source/Audio",
...
/*lint -e(123)*/GST_BOILERPLATE (GstAudioTestSrc, gst_audio_test_src, GstBaseSrc, GST_TYPE_BASE_SRC)
...
static void gst_audio_test_src_set_property(GObject * object, guint prop_id,
        const GValue * value, GParamSpec * pspec);
static void gst_audio_test_src_get_property(GObject * object, guint prop_id,
...
static void gst_audio_test_src_base_init(gpointer g_class)
{
...
static void gst_audio_test_src_class_init(GstAudioTestSrcClass * klass)
{
    ...
    g_object_class_install_property(gobject_class, PROP_WAVE,
            g_param_spec_enum("wave", "Waveform", "Oscillator waveform",
                    GST_TYPE_AUDIO_TEST_SRC_WAVE, /* enum type */
                    GST_AUDIO_TEST_SRC_WAVE_SINE, /* default value */
                    G_PARAM_READWRITE | GST_PARAM_CONTROLLABLE
                            | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_class, PROP_FREQ,
            g_param_spec_double("freq", "Frequency",
                    "Frequency of test signal", 0.0, 20000.0, 440.0,
                    G_PARAM_READWRITE | GST_PARAM_CONTROLLABLE
                            | G_PARAM_STATIC_STRINGS));
    ...
    gstbasesrc_class->is_seekable = GST_DEBUG_FUNCPTR(
            gst_audio_test_src_is_seekable);
    ...
    gstbasesrc_class->get_times = GST_DEBUG_FUNCPTR(
            gst_audio_test_src_get_times);
    gstbasesrc_class->create = GST_DEBUG_FUNCPTR(gst_audio_test_src_create);
}

static void gst_audio_test_src_init(GstAudioTestSrc * src,
        GstAudioTestSrcClass * g_class)
{
...
static void gst_audio_test_src_src_fixate(GstPad * pad, GstCaps * caps)
{
    ...
    (void) gst_structure_fixate_field_nearest_int(structure, "rate",
            src->samplerate);

    name = gst_structure_get_name(structure);
    if (strcmp(name, "audio/x-raw-int") == 0)
        (void) gst_structure_fixate_field_nearest_int(structure, "width", 32);
    else if (strcmp(name, "audio/x-raw-float") == 0)
        (void) gst_structure_fixate_field_nearest_int(structure, "width", 64);
}

static gboolean gst_audio_test_src_setcaps(GstBaseSrc * basesrc, GstCaps * caps)
{
...
static gboolean gst_audio_test_src_query(GstBaseSrc * basesrc, GstQuery * query)
{
    GstAudioTestSrc *src = GST_AUDIO_TEST_SRC(basesrc);
    gboolean res = FALSE;
 ...
 * Many thanks Phil!
 */
static void gst_audio_test_src_init_pink_noise(GstAudioTestSrc * src)
{
    gint i;
...
static gdouble gst_audio_test_src_generate_pink_noise_value(GstPinkNoise * pink)
{
    glong new_random;
    glong sum;
    ...
    if (pink->index != 0)
    {
        ...
        new_random = (glong)(32768.0 - (65536.0 * (gulong) rand() / (RAND_MAX
                + 1.0)));
    ...
    new_random = (glong)(32768.0 - (65536.0 * (gulong) rand()
            / (RAND_MAX + 1.0)));
...
static void gst_audio_test_src_init_sine_table(GstAudioTestSrc * src)
{
    gint i;
    gdouble ang = 0.0;
...
DEFINE_SINE_TABLE (int16, 32767)
DEFINE_SINE_TABLE (int32, 2147483647)
 ...
 */
static void gst_audio_test_src_change_volume(GstAudioTestSrc * src)
{
    switch (src->wave)
    {
    case GST_AUDIO_TEST_SRC_WAVE_SINE_TAB:
        ...
        GstSegment * segment)
{
    ...
    src->n_samples = gst_util_uint64_scale_int(gstClockTime, src->samplerate,
            GST_SECOND);
    ...
    {
        gstClockTime = segment->stop;
        src->n_samples_stop = gst_util_uint64_scale_int(gstClockTime,
                src->samplerate, GST_SECOND);
        ...
        guint64 offset, guint length, GstBuffer ** buffer)
{
    ...
    if (!src->tags_pushed)
    {
        ...
        gst_tag_list_add(taglist, GST_TAG_MERGE_APPEND, GST_TAG_DESCRIPTION,
                "audiotest wave", NULL);

        event = gst_event_new_tag(taglist);
        src->tags_pushed = gst_pad_push_event(basesrc->srcpad, event);
    }

    /* check for eos */
    if (src->check_seek_stop && (src->n_samples_stop > src->n_samples)
    ...
    if ((res = gst_pad_alloc_buffer(basesrc->srcpad, src->n_samples,
            src->generate_samples_per_buffer * sample_size, GST_PAD_CAPS(
                    basesrc->srcpad), &buf)) != GST_FLOW_OK)
    {
        return res;
    }
